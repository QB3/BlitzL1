                                            ...
                                            const Dataset *data)
  {
    ...
    for (int cd_itr = 0; cd_itr < max_cd_itr; ++cd_itr)
    {
      ...
      for (size_t ind = 0; ind < working_set_size; ++ind)
      {
        ...
        value_t grad = prox_newton_grad_cache[ind] + col->weighted_inner_product(A_Delta_x, H);
    ...
    for (size_t ind = 0; ind < working_set_size; ++ind)
    {
      ...
      value_t approximate_grad = prox_newton_grad_cache[ind] + col->weighted_inner_product(A_Delta_x, H);
